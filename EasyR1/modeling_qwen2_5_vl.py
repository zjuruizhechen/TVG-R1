        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                if n_video_tokens != n_video_features:
                    # 如果 video_embeds 的数量多于 video_tokens，裁剪 video_embeds
                    if n_video_features > n_video_tokens:
                        video_embeds = video_embeds[:n_video_tokens]
                    # 如果 video_embeds 的数量少于 video_tokens，填充 video_embeds
                    elif n_video_features < n_video_tokens:
                        # 填充的值可以是零向量或其他合适的值
                        padding = torch.zeros(n_video_tokens - n_video_features, video_embeds.shape[1], 
                                            dtype=video_embeds.dtype, device=video_embeds.device)
                        video_embeds = torch.cat((video_embeds, padding), dim=0)
