...
def _merge_multimodal_embeddings(
    ...
    For extra placeholders, assign a default embedding (zeros by default).
    """
    num_expected_tokens = is_multimodal.sum().item()
    assert isinstance(num_expected_tokens, int)

    flattened = _flatten_embeddings(multimodal_embeddings)
    num_actual_tokens = flattened.shape[0]
    embed_dim = inputs_embeds.shape[-1]

    # For extra placeholders, fill with default_embed (zero by default)
    if num_actual_tokens < num_expected_tokens:
        if default_embed is None:
            # Use zero vector if not specified
            default_embed = torch.zeros((embed_dim,), device=inputs_embeds.device, dtype=inputs_embeds.dtype)
        # Concatenate zeros for missing tokens
        padding = default_embed.unsqueeze(0).expand(num_expected_tokens - num_actual_tokens, -1)
        flattened = torch.cat([flattened, padding], dim=0)
    elif num_actual_tokens > num_expected_tokens:
        # Truncate extra embeddings
        flattened = flattened[:num_expected_tokens]

    # Now shape matches
    assert flattened.shape[0] == num_expected_tokens
