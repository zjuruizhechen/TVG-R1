...
def tvg_format_reward(predict_str: str) -> float:
    pattern = re.compile(r"<think>.*</think>.*<time>.*</time>.*\\boxed\{.*\}.*", re.DOTALL)
...
def tvg_compute_score(predict_str: str, ground_truth: list, gt_frame: list, video_length: float) -> float:
    ...
    reward = 0.7 * acc_reward + 0.1 * format_reward + 0.2 * tvg_reward
    # reward /= 2
    return reward
