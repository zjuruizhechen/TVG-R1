...
def tvg_format_reward(predict_str: str) -> float:
    pattern = re.compile(r"<think>.*</think>.*<time>.*</time>.*", re.DOTALL)
...
def tvgonly_compute_score(predict_str: str, ground_truth: list, gt_frame: list, video_length: float) -> float:
