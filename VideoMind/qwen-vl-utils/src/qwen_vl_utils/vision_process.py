...
def smart_nframes(
    ...
    if not (FRAME_FACTOR <= nframes and nframes <= total_frames):
        ...
        nframes = max(min(nframes, total_frames), FRAME_FACTOR)
    return nframes
...
def _read_video_torchvision(
    ...
    try:
        video, audio, info = io.read_video(
            video_path,
            start_pts=int(ele.get("video_start", 0.0)),
            end_pts=video_end,
            pts_unit="sec",
            output_format="TCHW",
        )
...
def _read_video_decord(
    ...
    if start_frame > end_frame:
        start_frame, end_frame = end_frame, start_frame  # 保证顺序
    ...
    try:
        ...
        raise RuntimeError(f"Failed to read frames {idx} from video {video_path}. Reason: {str(e)}")
...
#     """
#     import decord
#     decord.logging.set_level(decord.logging.ERROR)  # 或 WARNING / SILENT
#     video_path = ele["video"]
#     st = time.time()
#     vr = decord.VideoReader(video_path)
#     total_frames, video_fps = len(vr), vr.get_avg_fps()
